...
st.set_page_config(page_title="Tonkotsu Ramen — SAVOUR", page_icon="🍽️", layout="wide")
st.markdown("""<style>
...
st.markdown('<div class="recipe-hero"><div class="recipe-emoji">🍜</div><div class="recipe-title">Tonkotsu Ramen</div><div class="recipe-tag">Japanisch</div><div class="divider"></div></div>', unsafe_allow_html=True)
st.markdown('<div class="img-placeholder-large">📸</div>', unsafe_allow_html=True)
c1,c2,c3=st.columns(3)
with c1: st.markdown('<div class="info-card"><div class="info-num">36 Std</div><div class="info-label">Zubereitungszeit</div></div>', unsafe_allow_html=True)
...
with c3: st.markdown('<div class="info-card"><div class="info-num">650 kcal</div><div class="info-label">Pro Portion</div></div>', unsafe_allow_html=True)
col1,col2=st.columns([3,2])
with col1:
    st.markdown('<div class="section-head">Zubereitung</div>', unsafe_allow_html=True)
    st.markdown('<div class="step-item"><span class="step-num">1</span>Knochen 1 Stunde in kaltem Wasser einweichen, dann abkochen und abspülen.</div>', unsafe_allow_html=True)
    st.markdown('<div class="step-item"><span class="step-num">2</span>In frischem Wasser mit Knoblauch und Ingwer aufkochen, dann 12-36 Stunden bei mittlerer Hitze köcheln.</div>', unsafe_allow_html=True)
    st.markdown('<div class="step-item"><span class="step-num">3</span>Regelmäßig Wasser nachgießen und umrühren bis die Brühe milchig-weiß wird.</div>', unsafe_allow_html=True)
    st.markdown('<div class="step-item"><span class="step-num">4</span>Chashu: Schweinebauch einrollen, mit Sojasauce und Mirin 3 Stunden schmoren.</div>', unsafe_allow_html=True)
    st.markdown('<div class="step-item"><span class="step-num">5</span>Ajitama: Eier 6,5 Min kochen, in Sojasauce-Mirin-Marinade über Nacht einlegen.</div>', unsafe_allow_html=True)
    st.markdown('<div class="step-item"><span class="step-num">6</span>Brühe abseihen, mit Tare (Sojasauce-Basis) abschmecken.</div>', unsafe_allow_html=True)
    st.markdown('<div class="step-item"><span class="step-num">7</span>Nudeln kochen, in Schüssel mit Brühe anrichten, Chashu, Ei, Nori und Frühlingszwiebeln darauf.</div>', unsafe_allow_html=True)
with col2:
    st.markdown('<div class="section-head">Zutaten</div>', unsafe_allow_html=True)
    st.markdown('<div class="ingredient-item">• 2kg Schweineknochen</div>', unsafe_allow_html=True)
    st.markdown('<div class="ingredient-item">• 500g Schweinebauch (für Chashu)</div>', unsafe_allow_html=True)
    st.markdown('<div class="ingredient-item">• 4 Portionen Ramen-Nudeln</div>', unsafe_allow_html=True)
    st.markdown('<div class="ingredient-item">• 4 Eier (für Ajitama)</div>', unsafe_allow_html=True)
    st.markdown('<div class="ingredient-item">• Sojasauce, Mirin, Sake</div>', unsafe_allow_html=True)
    st.markdown('<div class="ingredient-item">• Frühlingszwiebeln</div>', unsafe_allow_html=True)
    st.markdown('<div class="ingredient-item">• Nori-Blätter</div>', unsafe_allow_html=True)
    st.markdown('<div class="ingredient-item">• Knoblauch, Ingwer</div>', unsafe_allow_html=True)
    st.markdown('<div class="ingredient-item">• Sesamöl, weißer Pfeffer</div>', unsafe_allow_html=True)
    st.markdown('<div class="section-head">Nährwerte</div>', unsafe_allow_html=True)
    st.markdown('<div class="nutrition-card"><div class="nutrition-row"><span>Kalorien</span><span class="nutrition-val">650 kcal</span></div><div class="nutrition-row"><span>Protein</span><span class="nutrition-val">38g</span></div><div class="nutrition-row"><span>Kohlenhydrate</span><span class="nutrition-val">55g</span></div><div class="nutrition-row"><span>Fett</span><span class="nutrition-val">32g</span></div></div>', unsafe_allow_html=True)
