...
st.set_page_config(page_title="Basque Cheesecake — SAVOUR", page_icon="🍽️", layout="wide")
st.markdown("""<style>
...
st.markdown('<div class="recipe-hero"><div class="recipe-emoji">🍰</div><div class="recipe-title">Basque Cheesecake</div><div class="recipe-tag">Spanisch</div><div class="divider"></div></div>', unsafe_allow_html=True)
st.markdown('<div class="img-placeholder-large">📸</div>', unsafe_allow_html=True)
c1,c2,c3=st.columns(3)
with c1: st.markdown('<div class="info-card"><div class="info-num">1 Std</div><div class="info-label">Zubereitungszeit</div></div>', unsafe_allow_html=True)
...
with c3: st.markdown('<div class="info-card"><div class="info-num">450 kcal</div><div class="info-label">Pro Portion</div></div>', unsafe_allow_html=True)
col1,col2=st.columns([3,2])
with col1:
    st.markdown('<div class="section-head">Zubereitung</div>', unsafe_allow_html=True)
    st.markdown('<div class="step-item"><span class="step-num">1</span>Ofen auf 220°C vorheizen. Springform (20cm) mit Backpapier auslegen, überstehen lassen.</div>', unsafe_allow_html=True)
    st.markdown('<div class="step-item"><span class="step-num">2</span>Frischkäse mit Zucker cremig rühren bis keine Klumpen mehr da sind.</div>', unsafe_allow_html=True)
    st.markdown('<div class="step-item"><span class="step-num">3</span>Eier einzeln unterrühren, jeweils gut einarbeiten.</div>', unsafe_allow_html=True)
    st.markdown('<div class="step-item"><span class="step-num">4</span>Sahne und Vanille dazugeben, glatt rühren.</div>', unsafe_allow_html=True)
    st.markdown('<div class="step-item"><span class="step-num">5</span>Mehl und Salz sieben und vorsichtig unterheben.</div>', unsafe_allow_html=True)
    st.markdown('<div class="step-item"><span class="step-num">6</span>In die Form gießen und 50-55 Minuten backen bis die Oberfläche dunkelbraun ist.</div>', unsafe_allow_html=True)
    st.markdown('<div class="step-item"><span class="step-num">7</span>Komplett abkühlen lassen, dann mindestens 4 Stunden kalt stellen. Die Mitte soll noch wackeln!</div>', unsafe_allow_html=True)
with col2:
    st.markdown('<div class="section-head">Zutaten</div>', unsafe_allow_html=True)
    st.markdown('<div class="ingredient-item">• 600g Frischkäse (Raumtemperatur)</div>', unsafe_allow_html=True)
    st.markdown('<div class="ingredient-item">• 200g Zucker</div>', unsafe_allow_html=True)
    st.markdown('<div class="ingredient-item">• 4 große Eier</div>', unsafe_allow_html=True)
    st.markdown('<div class="ingredient-item">• 300ml Sahne</div>', unsafe_allow_html=True)
    st.markdown('<div class="ingredient-item">• 1 EL Vanilleextrakt</div>', unsafe_allow_html=True)
    st.markdown('<div class="ingredient-item">• 30g Mehl</div>', unsafe_allow_html=True)
    st.markdown('<div class="ingredient-item">• Prise Salz</div>', unsafe_allow_html=True)
    st.markdown('<div class="section-head">Nährwerte</div>', unsafe_allow_html=True)
    st.markdown('<div class="nutrition-card"><div class="nutrition-row"><span>Kalorien</span><span class="nutrition-val">450 kcal</span></div><div class="nutrition-row"><span>Protein</span><span class="nutrition-val">12g</span></div><div class="nutrition-row"><span>Kohlenhydrate</span><span class="nutrition-val">28g</span></div><div class="nutrition-row"><span>Fett</span><span class="nutrition-val">34g</span></div></div>', unsafe_allow_html=True)
