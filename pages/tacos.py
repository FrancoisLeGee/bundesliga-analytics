...
st.set_page_config(page_title="Tacos al Pastor — SAVOUR", page_icon="🍽️", layout="wide")
st.markdown("""<style>
...
st.markdown('<div class="recipe-hero"><div class="recipe-emoji">🌮</div><div class="recipe-title">Tacos al Pastor</div><div class="recipe-tag">Mexikanisch</div><div class="divider"></div></div>', unsafe_allow_html=True)
st.markdown('<div class="img-placeholder-large">📸</div>', unsafe_allow_html=True)
c1,c2,c3=st.columns(3)
with c1: st.markdown('<div class="info-card"><div class="info-num">45 Min</div><div class="info-label">Zubereitungszeit</div></div>', unsafe_allow_html=True)
...
with c3: st.markdown('<div class="info-card"><div class="info-num">420 kcal</div><div class="info-label">Pro Portion</div></div>', unsafe_allow_html=True)
col1,col2=st.columns([3,2])
with col1:
    st.markdown('<div class="section-head">Zubereitung</div>', unsafe_allow_html=True)
    st.markdown('<div class="step-item"><span class="step-num">1</span>Chilis einweichen, mit Chipotle, Knoblauch, Kreuzkümmel und Orangensaft pürieren.</div>', unsafe_allow_html=True)
    st.markdown('<div class="step-item"><span class="step-num">2</span>Fleisch in der Marinade mindestens 2 Stunden einlegen.</div>', unsafe_allow_html=True)
    st.markdown('<div class="step-item"><span class="step-num">3</span>Ananas in Scheiben schneiden und in der Pfanne karamellisieren.</div>', unsafe_allow_html=True)
    st.markdown('<div class="step-item"><span class="step-num">4</span>Fleisch bei hoher Hitze scharf anbraten bis knusprig an den Rändern.</div>', unsafe_allow_html=True)
    st.markdown('<div class="step-item"><span class="step-num">5</span>In kleine Stücke schneiden, mit karamellisierter Ananas mischen.</div>', unsafe_allow_html=True)
    st.markdown('<div class="step-item"><span class="step-num">6</span>Tortillas kurz in der trockenen Pfanne erwärmen.</div>', unsafe_allow_html=True)
    st.markdown('<div class="step-item"><span class="step-num">7</span>Tortillas füllen, mit Zwiebel, Koriander, Limette und Salsa Verde servieren.</div>', unsafe_allow_html=True)
with col2:
    st.markdown('<div class="section-head">Zutaten</div>', unsafe_allow_html=True)
    st.markdown('<div class="ingredient-item">• 500g Schweineschulter (dünn geschnitten)</div>', unsafe_allow_html=True)
    st.markdown('<div class="ingredient-item">• 4 getrocknete Guajillo-Chilis</div>', unsafe_allow_html=True)
    st.markdown('<div class="ingredient-item">• 2 Chipotle in Adobo</div>', unsafe_allow_html=True)
    st.markdown('<div class="ingredient-item">• 1 frische Ananas</div>', unsafe_allow_html=True)
    st.markdown('<div class="ingredient-item">• Kleine Maistortillas</div>', unsafe_allow_html=True)
    st.markdown('<div class="ingredient-item">• 1 weiße Zwiebel</div>', unsafe_allow_html=True)
    st.markdown('<div class="ingredient-item">• Frischer Koriander</div>', unsafe_allow_html=True)
    st.markdown('<div class="ingredient-item">• 3 Limetten</div>', unsafe_allow_html=True)
    st.markdown('<div class="ingredient-item">• Salsa Verde</div>', unsafe_allow_html=True)
    st.markdown('<div class="section-head">Nährwerte</div>', unsafe_allow_html=True)
    st.markdown('<div class="nutrition-card"><div class="nutrition-row"><span>Kalorien</span><span class="nutrition-val">420 kcal</span></div><div class="nutrition-row"><span>Protein</span><span class="nutrition-val">26g</span></div><div class="nutrition-row"><span>Kohlenhydrate</span><span class="nutrition-val">38g</span></div><div class="nutrition-row"><span>Fett</span><span class="nutrition-val">18g</span></div></div>', unsafe_allow_html=True)
