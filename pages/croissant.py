...
st.set_page_config(page_title="Butter-Croissant — SAVOUR", page_icon="🍽️", layout="wide")
st.markdown("""<style>
...
st.markdown('<div class="recipe-hero"><div class="recipe-emoji">🥐</div><div class="recipe-title">Butter-Croissant</div><div class="recipe-tag">Französisch</div><div class="divider"></div></div>', unsafe_allow_html=True)
st.markdown('<div class="img-placeholder-large">📸</div>', unsafe_allow_html=True)
c1,c2,c3=st.columns(3)
with c1: st.markdown('<div class="info-card"><div class="info-num">4 Std</div><div class="info-label">Zubereitungszeit</div></div>', unsafe_allow_html=True)
with c2: st.markdown('<div class="info-card"><div class="info-num">8</div><div class="info-label">Portionen</div></div>', unsafe_allow_html=True)
with c3: st.markdown('<div class="info-card"><div class="info-num">320 kcal</div><div class="info-label">Pro Portion</div></div>', unsafe_allow_html=True)
col1,col2=st.columns([3,2])
with col1:
    st.markdown('<div class="section-head">Zubereitung</div>', unsafe_allow_html=True)
    st.markdown('<div class="step-item"><span class="step-num">1</span>Mehl, Salz, Zucker und Hefe mischen. Kalte Milch dazu, 10 Min kneten bis glatter Teig entsteht.</div>', unsafe_allow_html=True)
    st.markdown('<div class="step-item"><span class="step-num">2</span>Teig 1 Stunde im Kühlschrank ruhen lassen.</div>', unsafe_allow_html=True)
    st.markdown('<div class="step-item"><span class="step-num">3</span>Butter zwischen Backpapier zu einem Quadrat (15x15cm) ausrollen.</div>', unsafe_allow_html=True)
    st.markdown('<div class="step-item"><span class="step-num">4</span>Teig ausrollen, Butter einschlagen, dreimal tourieren mit je 30 Min Kühlung dazwischen.</div>', unsafe_allow_html=True)
    st.markdown('<div class="step-item"><span class="step-num">5</span>Teig 3mm dünn ausrollen, Dreiecke schneiden und von der breiten Seite aufrollen.</div>', unsafe_allow_html=True)
    st.markdown('<div class="step-item"><span class="step-num">6</span>1 Stunde gehen lassen bis doppelte Größe, mit Ei bestreichen.</div>', unsafe_allow_html=True)
    st.markdown('<div class="step-item"><span class="step-num">7</span>Bei 200°C ca. 15 Minuten goldbraun backen.</div>', unsafe_allow_html=True)
with col2:
    st.markdown('<div class="section-head">Zutaten</div>', unsafe_allow_html=True)
    st.markdown('<div class="ingredient-item">• 500g Mehl (Type 550)</div>', unsafe_allow_html=True)
    st.markdown('<div class="ingredient-item">• 300g kalte Butter</div>', unsafe_allow_html=True)
    st.markdown('<div class="ingredient-item">• 10g Salz</div>', unsafe_allow_html=True)
    st.markdown('<div class="ingredient-item">• 80g Zucker</div>', unsafe_allow_html=True)
    st.markdown('<div class="ingredient-item">• 10g frische Hefe</div>', unsafe_allow_html=True)
    st.markdown('<div class="ingredient-item">• 280ml kalte Milch</div>', unsafe_allow_html=True)
    st.markdown('<div class="ingredient-item">• 1 Ei (zum Bestreichen)</div>', unsafe_allow_html=True)
    st.markdown('<div class="section-head">Nährwerte</div>', unsafe_allow_html=True)
    st.markdown('<div class="nutrition-card"><div class="nutrition-row"><span>Kalorien</span><span class="nutrition-val">320 kcal</span></div><div class="nutrition-row"><span>Protein</span><span class="nutrition-val">6g</span></div><div class="nutrition-row"><span>Kohlenhydrate</span><span class="nutrition-val">35g</span></div><div class="nutrition-row"><span>Fett</span><span class="nutrition-val">18g</span></div></div>', unsafe_allow_html=True)
